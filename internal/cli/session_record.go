...
func sessionRecordFromNormalized(session normalizedSession, workspaceRoot string, since, generatedAt time.Time, ownership ownershipCatalog) (codexSessionRecord, error) {
	...
	for _, event := range session.Events {
		...
		if event.RepositoryScopeKnown {
			inRepositoryScope = event.InRepositoryScope
		} else if inside, explicit := eventRepositoryScope(event, session.CWD, workspaceRoot); explicit {
			inRepositoryScope = inside
		}
		...
		case sessionEventToolCall:
			...
			if !event.OperationAttributionAmbiguous {
				...
				for _, flag := range eventFlags {
					...
					touchSessionActivity(record.Activity, "owned-flag", flag, event.OccurredAt)
				}
			}
